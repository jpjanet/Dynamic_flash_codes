function  [fn] = flash_postprocessing( tt1,Mi,x1,y1,Ml,Mv,M1,L1,V1,T1,P1,Pout1,F1,w1,rho_R,tt2,x2,y2,M2,L2,V2,T2,P2,F2,w2,testcase,comp_string ,integrator,imageF,print_figs,fn);
...
%% Save figures:
if print_figs
disp(['Saving fgiures as ',imageF, ' in CD'])
else
disp('Not saving fgiures')
end
...
title(['Number of timesteps, testcase = ',num2str(testcase),', integrator = ',integrator])
xlabel('Time, t [h]')
...
axis tight
title(['Dimensionless holdups, test case  = ',num2str(testcase)])
xlabel('Time, t [h]')
